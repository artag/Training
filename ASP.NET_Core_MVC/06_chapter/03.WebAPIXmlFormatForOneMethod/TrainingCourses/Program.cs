...
// (2) Добавление сериализации данных в XML.
//     Можно включать формат XML для отдельных методов (через арибут 'Produces').
// (3) Опция отключает validation для модели Course перед вызовом метода контроллера.
...
builder.Services
    ...
    {
        options.SuppressModelStateInvalidFilter = true;     // (3)
    });
