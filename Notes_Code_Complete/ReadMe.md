...
*Связность* (м.б. «сила» (strength)) характеризует соответствие выполняемых в методе
...
* **Логическая связность** (logical cohesion) - метод включает несколько операций, а
...
* Не ограничивайте длину имен методов искусственными правилам
...
* Передавайте в метод те переменные или объекты, которые нужны ему для поддержания
  ...
  Если объект ну никак "не подходит" к методу, то лучше передавать отдельные параметры
  вместо него.
...
* Имеют ли методы слабое сопряжение? Являются ли связи метода с другими методами
...
Если код в некотором методе встречает неожиданную ситуацию и не знает, как ее обработать,
...
Всегда следует принимать во внимание все возможные методы обработки ошибок:
...
Подумайте, действительно ли вашей программе необходимо обрабатывать исключения.

### 8.5. Изоляция повреждений, вызванных ошибками

*Изоляция повреждений*, или *баррикада*, — это стратегия, сходная с тем, как изолируются
отсеки в трюме корабля.

Один из способов изоляции - разработка набора интерфейсов в качестве оболочки для
«безопасных» частей кода. Проверяйте корректность данных, пересекающих границу безопасной
области, и реагируйте соответственно, если данные неправильные.

```
Ненадежные       Проверочный класс 1      Внутренние классы,
("внешние") ---> Проверочный класс 2 ---> работают с проверенными данными.
данные           Проверочный класс 3
```

Тот же подход применим и на уровне класса. Открытые методы класса предполагают, что
данные небезопасны и отвечают за их проверку и исправление. Если данные были проверены
открытыми методами класса, закрытые методы могут считать, что данные безопасны.

Решение, какой код находится внутри, а какой — снаружи баррикады, принимается на уровне
архитектуры.

* Преобразовывайте входные данные к нужному типу в момент ввода

Сохранение данных неопределенного типа в течение неизвестного периода времени усложняет
программу.
...
|Промышленная версия                | Отладочная версия                      |
|-----------------------------------|----------------------------------------|
| Должна работать быстро            | Может позволить себе работать медленно |
| Должна быть экономной с ресурсами | Может быть расточительной              |
| Не должна позволять пользователю опасные действия | Могут быть доп. средства для отладки |

#### Внедрите поддержку отладки как можно раньше

Чем раньше вы добавите отладочные средства, тем больше они помогут.

#### Используйте наступательное программирование

Исключительные случаи должны обрабатываться так, что-бы во время разработки они были очевидны, а в промышленном коде — позволяли продолжить работу.

Некоторые приемы:
* Утверждения завершают работу программы.
...
* При попадании в ветвь `default` или `else` всех операторов `case` программа прекращает
  работу или еще как-то заставляет обратить на это внимание.
* Заполняйте объекты мусором перед самым их удалением.
* Настройте программу на отправку вам журналов ошибок по email (если это можно сделать).

#### Запланируйте удаление отладочных средств

* Для контроля версий и сборки программы используйте инструменты `ant` и `make`
  * Средства контроля версий позволяют создавать варианты программы из одних и тех же
    исходных файлов.
  * Инструменты для сборки позволяют настроить включение отладочного кода в режиме
    разработки и его исключение в коммерческой версии.

* Используйте встроенный препроцессор

  Если в вашей программной среде есть препроцессор, (как в C++), вы можете добавлять или
  удалять отладочный код простым изменением параметра компиляции.

  Препроцессор можно задействовать непосредственно, а можно писать макросы, работающие с
  его определениями.

  Пример непосредственного использования препроцессора (C++):
  ```cpp
  #define DEBUG
  ...
  #if defined( DEBUG )
  ...
  Пример использования макроса препроцессора для управления отладочным кодом (C++):
  ```cpp
  #define DEBUG
  #if defined( DEBUG )
...
* Напишите собственный препроцессор (если язык не содержит препроцессор)

* Используйте отладочные заглушки

  Для выполнения отладочных проверок можно вызвать процедуру. Во время разработки она
  может выполнить несколько операций перед тем, как управление вернется вызывающей
  стороне.

  В промышленном коде сложную процедуру можно заменить процедурой-заглушкой, которая
  сразу вернет управление или выполнит перед этим пару быстрых операций.

  Храните и отладочную, и итоговую версии процедур, и вы сможете быстро переключаться
  между ними.

### 8.7. Доля защитного программирования в промышленной версии

Основные принципы для определения, какие инструменты защитного программирования следует
оставить в промышленной версии, а какие — убрать.

* Оставьте код, которые проверяет существенные ошибки

  Решите, какие части программы могут содержать скрытые ошибки, а какие — нет.

  Пример для электронной таблицы: можно скрыть ошибки, касающиеся обновления экрана, но
  нельзя скрыть ошибки в вычислительном модуле.

* Удалите код, проверяющий незначительные ошибки

  Если последствия ошибки действительно незначительны, удалите код, который ее проверяет.
  «Удалить» значит не обязательно физически убрать код, можно использовать управление
  версиями, условную компиляцию и т.п.

  Проверочный код можно оставить, но настроив для ненавязчивой записи сообщений в журнал
  ошибок.
...
* Оставьте код, который позволяет аккуратно завершить работу программы
  ...
  Если программа содержит отладочный код, определяющий потенциально фатальные ошибки,
  оставьте его — это позволит элегантно завершить работу.

* Регистрируйте ошибки для отдела технической поддержки

  Можно оставить отладочные средства в промышленной версии, но изменить их поведение на
  более подходящее.

  Пример: оставить утверждения в "боевой" версии и настроить их на запись сообщений в
  файл.
...
Проблемы от чрезмерного защитного программирования:

* От чрезмерных проверок программа может получиться слишком большой и медленной.
* Дополнительный код, необходимый для защитного программирования, увеличивает сложность.
...
* Используете ли вы утверждения для документирования допущений, включая пред- и
  постусловия?

* Используются ли утверждения для документирования только тех условий, которые никогда не
  должны происходить?

* Определены ли в архитектуре или высокоуровневом проекте системы технологии обработки
  ошибок?

* Указано ли в архитектуре или высокоуровневом проекте системы, чему будет отдаваться
  предпочтение при обработке ошибок: устойчивости или корректности?

* Построены ли баррикады для изоляции разрушительного эффекта ошибок и уменьшения объема
  кода, занятого в обработке ошибок?

* Установлены ли отладочные средства таким образом, что их можно активизировать и
  деактивировать без особых проблем?

* Хватает ли защитного кода: не слишком много и не слишком мало?

* Использованы ли технологии наступательного программирования, чтобы затруднить пропуск
...
* Обрабатывается ли ошибка по возможности локально или генерируется нелокальное
  исключение?
...
* Свободен ли код от пустых блоков catch? (Или, если блок catch действительно допустим,
  задокументировано ли это?)

#### Вопросы безопасности

* Действительно ли код, проверяющий некорректные входные данные, контролирует попытки
  переполнения буфера, внедрения SQL- и HTML-кода, переполнения целых чисел и других
...
* Не содержат ли сообщения об ошибках информацию, которая может помочь злоумышленнику
  взломать систему?

### Ключевые моменты

* Промышленный код должен обрабатывать ошибки более изощренно, чем по принципу «мусор на
  входе — мусор на выходе».
* С помощью технологии защитного программирования ошибки легче находить, легче исправлять,
  и они наносят меньше вреда промышленному коду.
* Утверждения позволяют обнаружить ошибки на ранней стадии, особенно в больших системах,
  системах повышенной надежности и в системах с часто изменяемым кодом.
* Выбор способа обработки некорректных входных данных — это ключевое решение обработки
  ошибок, принимаемое на этапе высокоуровневого проектирования.
* Исключения предоставляют возможность обработки ошибок в измерении, отличном от
  нормального хода алгоритма. Если они используются с осторожностью, то являются важным
  дополнением в интеллектуальном инструментальном наборе программиста. Применять их
  следует после сравнения с другими технологиями обработки ошибок.
* Ограничения, применяемые к промышленной версии системы, не обязательно должны
  относиться и ко времени разработки. Пользуясь этим преимуществом, вы можете добавлять в
  отладочную версию любой код, помогающий быстро выявлять ошибки.
